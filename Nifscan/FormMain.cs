...
namespace Nifscan
{
    public partial class FormMain : Form
    {
        ...
        public static float verXval = 0;
        public static float verYval = 0;
        ...
        private void currentFile(string path)
        {
            if (File.Exists(path))
            {
                ...
                bytesFile = new byte[fileSize];
                FileStream fs = File.OpenRead(path);
                fs.Read(bytesFile, 0, fileSize);
                fs.Close();
                            ...
                            if (((blocksNamesList[i] == "NiTriShape" && (realBlockStart + blocksSizeList[i]) == (jump2 + 8)) || (blocksNamesList[i] == "BSLODTriShape" && (realBlockStart + blocksSizeList[i]) == (jump2 + 20))) && (dataBlock >= -1 && dataBlock < blocksCount) && (skinBlock >= -1 && skinBlock < blocksCount) && (shaderBlock >= -1 && shaderBlock < blocksCount) && (alphaBlock >= -1 && alphaBlock < blocksCount))
                            {
                                ...
                                if (dataBlock != -1)
                                {
                                    ...
                                    if (blocksNamesList[dataBlock] == "NiTriShapeData")
                                    {
                                        ...
                                        jump3 += 4;
                                        if (bytesFile[jump3] == 1 && (verX || verY || verZ))
                                        {
                                            jump3++;
                                            for (int j = 0; j < numVertices; j++)
                                            {
                                                if (verX)
                                                {
                                                    if (verXHard)
                                                    {
                                                        replaceBytesInFile(jump3, BitConverter.GetBytes(verXval));
                                                    }
                                                    else
                                                    {
                                                        replaceBytesInFile(jump3, BitConverter.GetBytes(BitConverter.ToSingle(bytesFile, jump3) + verXval));
                                                    }
                                                }
                                                jump3 += 4;
                                                if (verY)
                                                {
                                                    if (verYHard)
                                                    {
                                                        replaceBytesInFile(jump3, BitConverter.GetBytes(verYval));
                                                    }
                                                    else
                                                    {
                                                        replaceBytesInFile(jump3, BitConverter.GetBytes(BitConverter.ToSingle(bytesFile, jump3) + verYval));
                                                    }
                                                }
                                                jump3 += 4;
                                                if (verZ)
                                                {
                                                    if (verZHard)
                                                    {
                                                        replaceBytesInFile(jump3, BitConverter.GetBytes(verZval));
                                                    }
                                                    else
                                                    {
                                                        replaceBytesInFile(jump3, BitConverter.GetBytes(BitConverter.ToSingle(bytesFile, jump3) + verZval));
                                                    }
                                                }
                                                jump3 += 4;
                                            }
                                        ...
                                        long convert = BitConverter.ToUInt16(bytesFile, jump3);
                                        hasTangents = ((VectorFlags)convert & VectorFlags.Has_Tangents) != 0;
                                        bool hasUV = ((VectorFlags)convert & VectorFlags.Has_UV) != 0;
                                        jump3 += 6;
                                        ...
                                        if (hasUV)
                                        {
                                            jump3 = jump3 + (numVertices * 8);
                                        }
                                        int triangles = BitConverter.ToUInt16(bytesFile, jump3 + 6);
                                        jump3 = jump3 + 12;
                                        if (bytesFile[jump3] == 1)
                                        {
                                            jump3 = jump3 + (triangles * 6);
                                        }
                                        jump3++;
                                        int match = BitConverter.ToUInt16(bytesFile, jump3);
                                        jump3 = jump3 + 2;
                                        if (match > 0)
                                        {
                                    ...
                                    if (jump3 > realBlockStart && (realBlockStart + blocksSizeList[shaderBlock]) > jump3)
                                    {
                                        ...
                                        if (textureSetBlock != -1)
                                        {
                                            ...
                                            for (int j = 0; j < num; j++)
                                            {
                                                ...
                                                if (j == 1 && !String.IsNullOrEmpty(word))
                                                {
                                                    normal = word;
                                                }
                                                else if (j == 3 && !String.IsNullOrEmpty(word) && word.EndsWith("_p.dds", StringComparison.OrdinalIgnoreCase))
                                                {
                                        ...
                                        if (parallaxAdd && normal != null && texturesList.Exists(s => s.Equals(normal, StringComparison.OrdinalIgnoreCase)))
                                        {
                                            if (!parallaxInvert && bytesFile[realBlockStart] == 0 && !hasParallaxFlag && skinBlock == -1)
                                            {
                                                ...
                                                replaceBytesInFile(jump3, BitConverter.GetBytes(shaderFlags1 + 2048));
                                                shaderFlags1 += 2048;
                                            }
                                        ...
                                        if ((hasVAlphaFlag && (!hasVColors || (alphaBlock == -1 && !hasRefractionFlag))))
                                        {
                                            if (!checkBox4.Checked)
                                            {
                                            ...
                                            {
                                                replaceBytesInFile(jump3, BitConverter.GetBytes(shaderFlags1 - 8));
                                                shaderFlags1 -= 8;
                                            }
                                        }
                                        if ((checkBox6.Checked || checkBox7.Checked) && comboBox1.SelectedIndex != -1)
                                        {
                                            if (((ShadeFlags1)shaderFlags1 & (ShadeFlags1)(1 << comboBox1.SelectedIndex)) != 0)
                                            {
                                                ...
                                                {
                                                    replaceBytesInFile(jump3, BitConverter.GetBytes(shaderFlags1 - (1 << comboBox1.SelectedIndex)));
                                        ...
                                        if (hasVColors && vcRemove && vcEmpty && (bytesFile[realBlockStart] == 0 || bytesFile[realBlockStart] == 1))
                                        {
                                            if (hasVCFlag)
                                            {
                                                replaceBytesInFile(jump3, BitConverter.GetBytes(shaderFlags2 - 32));
                                                shaderFlags2 -= 32;
                                                hasVCFlag = false;
                                            }
                                            bytesFile[vcYesNo] = 0;
                                            ...
                                            {
                                                realBlockStart = realBlockStart - (numVertices * 16);
                                            }
                                            if (jump3 > vcStart)
                                            {
                                                jump3 = jump3 - (numVertices * 16);
                                        ...
                                        if ((checkBox8.Checked || checkBox9.Checked) && comboBox2.SelectedIndex != -1)
                                        {
                                            if (((ShadeFlags2)shaderFlags2 & (ShadeFlags2)(1 << comboBox2.SelectedIndex)) != 0)
                                            {
                                                ...
                                                {
                                                    replaceBytesInFile(jump3, BitConverter.GetBytes(shaderFlags2 - (1 << comboBox2.SelectedIndex)));
                ...
                {
                    outLog.Add("WARNING! HEADER READ ERROR: " + fileName + " SOME REASON: " + headerError);
                }
            }
        }
        ...
        public void buttonColorParallax(bool edit)
        {
            buttonColor(button3, edit);
        }

        public void buttonColorEC(bool edit)
        {
            buttonColor(button4, edit);
        }

        public void buttonColorVC(bool edit)
        {
            buttonColor(button5, edit);
        }

        public void buttonColorVer(bool edit)
        {
            buttonColor(button6, edit);
        }

        private void buttonColor(Button button, bool edit)
        {
            if (edit)
            {
